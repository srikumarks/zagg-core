...
UtxoPaymentOpFrame::UtxoPaymentOpFrame(Operation const& op, OperationResult& res,
                               TransactionFrame& parentTx)
    : OperationFrame(op, res, parentTx), mPayment(mOperation.body.utxoPaymentOp())
{
}

bool
UtxoPaymentOpFrame::doApply(Application& app, LedgerDelta& delta,
                        LedgerManager& ledgerManager)
{
    ...
    {
        app.getMetrics()
            .NewMeter({"op-utxo-payment", "success", "apply"}, "operation")
            .Mark();
        innerResult().code(UTXO_PAYMENT_SUCCESS);
        return true;
    }
...
bool
UtxoPaymentOpFrame::doCheckValid(Application& app)
{
